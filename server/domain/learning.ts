...
export const MVP_STEPS: LearningNodeStepSeed[] = MVP_NODES.flatMap((node) => {
  ...
  return [
    ...
    },
    {
      ...
      description: "Aplique o que aprendeu em uma situação próxima da vida real.",
      content: { kind: "application" as const, instruction: "Escolha ou monte a resposta que mantém a intenção da conversa.", activityIds: [] },
    },
    {
      id: `${node.id}-review`,
      nodeId: node.id,
